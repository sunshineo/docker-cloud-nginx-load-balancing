...
export function parseServices(services) {
  ...
  services.forEach((container) => {
    ...
    const serverName = find(envvars, {key: "NGINX_SERVER_NAME"}).value
    let locationStr = "/"
    ...
    if (cert) {
      ...
      let sslServer = find(sslServers, { serverName: serverName })
      if (!sslServer) {
        console.log("Did not find existing ssl server using " + serverName + " in sslServers:")
        console.log(sslServers)
        sslServer = {
      ...
      let existingSslLocation = find(sslServer.locations, { locationStr: locationStr })
      if (!existingSslLocation) {
        sslServer.locations.push(location)
      }

      let plainServerRedirect = find(plainServers, { serverName: serverName })
      if (!plainServerRedirect) {
        console.log("Did not find existing plainServer server using " + serverName + " in plainServers:")
        console.log(plainServers)
        plainServerRedirect = {
          serverName: serverName,
          locations: [],
          redirectLocations: []
        }
        plainServers.push(plainServerRedirect)
      }
      let existingPlainLocation = find(plainServerRedirect.redirectLocations, { locationStr: locationStr })
      if (!existingPlainLocation) {
        plainServerRedirect.redirectLocations.push(location)
      }
    }
    else {
      let plainServer = find(plainServers, { serverName: serverName })
      if (!plainServer) {
        console.log("Did not find existing plainServer server using " + serverName + " in plainServers:")
        console.log(plainServers)
        plainServer = {
      ...
      let existingLocation = find(plainServer.locations, { locationStr: locationStr })
      if (!existingLocation) {
