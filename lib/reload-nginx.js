...
export function parseServices(services) {
  ...
  services.forEach((container) => {
    let envvars = container.container_envvars;
    const certs = find(envvars, {key: "NGINX_CERTS"})
    ...
    //and return {host, ssl}
    const virtualHosts = find(envvars, {key: "NGINX_VIRTUAL_HOST"}).value
      .split(",")
      .map((host, i) => {
        let ssl = false
      ...
      .forEach((virtualHost) => {
        ...
        if (!server) {
          ...
          let location = find(locations, { locationStr })
          if (!location) {
            location = {
              upstreamName: upstreamName,
              locationStr: locationStr
